`timescale 1ns/1ns
module sdfunit3 #(parameter WIDTH = 12, DEPTH = 7)(
    input clk,
    input di_en,
    input reset,
...
// ------------------------ signal control ------------------------
signal_control s0 (.clk(clk), .reset(reset), .di_en(di_en), .sel(sel), .bf_nj(bf_nj), .do_en(do_en));
// ------------------------ input ------------------------
